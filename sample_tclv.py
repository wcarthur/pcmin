...
def loadTrackFile(trackfile):
    ...
    """
    converters = {'datetime': lambda x: datetime.strptime(x, "%Y-%m-%d %H:%M:%S")}
    try:
...
def getidx(gridlon, gridlat, ptlon, ptlat, distance=500):
    ...
    msg = (f"Mean location of chosen points: "
           f"{np.mean(gridlon[idx])}E, "
           f"{np.mean(gridlat[idy])}S")
    LOGGER.debug(msg)
...
def sampleMonthlyLTMPI(dt, lon, lat, filepath, distance):
    ...
    if (lon > dslon.max()) or (lon < dslon.min()):
        LOGGER.warn(f"Point lies outside the data grid")
        return 0, 0
    if (lat > dslat.max()) or (lat < dslat.min()):
        LOGGER.warn(f"Point lies outside the data grid")
        return 0, 0

    idx, jdy = getidx(dslon, dslat, lon, lat, distance)
    monthidx = ds.groupby('time.month').groups
    
    vmax = ds.vmax.isel(time=monthidx[dt.month])[:, jdy, idx].mean(dim=['time', 'longitude', 'latitude'], skipna=True).values
    pmin = ds.pmin.isel(time=monthidx[dt.month])[:, jdy, idx].mean(dim=['time', 'longitude', 'latitude'], skipna=True).values

    LOGGER.debug(f"Monthly LTM Vmax: {vmax:.1f} m/s | Pmin {pmin:.1f} hPa")
    return vmax, pmin

def sampleMonthlyMaxPI(dt, lon, lat, filepath, distance):
    """
    Sample monthly long term mean PI
    ...
    :returns: Monthly mean potential intensity maximum wind speed 
              ...
              the observation
    """
    LOGGER.info(f"Extracting monthly long term max data for {dt.strftime('%Y-%m-%d %H:%M')}")
    ...
    vmax = ds.vmax.interp(time=dt.strftime("%Y-%m-%d"))[jdy, idx].mean(dim=['longitude', 'latitude'], skipna=True).values
    pmin = ds.pmin.interp(time=dt.strftime("%Y-%m-%d"))[jdy, idx].mean(dim=['longitude', 'latitude'], skipna=True).values

    LOGGER.debug(f"Monthly max Vmax: {vmax:.1f} m/s | Pmin {pmin:.1f} hPa")
    return vmax, pmin
...
def sampleDailyLTMPI(dt, lon, lat, filepath, distance):
    ...
    idx, jdy = getidx(dslon, dslat, lon, lat, distance)
    dayidx = ds.groupby('time.dayofyear').groups

    if isleap(dt.year):
        dt = dt.replace(year=dt.year-1)
    vmax = ds.vmax.isel(time=dayidx[dt.dayofyear])[:, jdy, idx].mean(dim=['time', 'longitude', 'latitude'], skipna=True).values
    pmin = ds.pmin.isel(time=dayidx[dt.dayofyear])[:, jdy, idx].mean(dim=['time', 'longitude', 'latitude'], skipna=True).values
...
def main():
    ...
    monthlyLTMaxPath = config.get('Input', 'MonthlyLTMmax')
    ...
    obstc['monthlymaxvmax'] = np.zeros(len(obstc.index))
    obstc['monthlymaxpmin'] = np.zeros(len(obstc.index))
    ...
    for idx, row in obstc.iterrows():
        ...
        # Monthly maximum
        vmax, pmin = sampleMonthlyMaxPI(row['datetime'], row['lon'], row['lat'], monthlyLTMaxPath, distance)
        obstc.loc[idx, 'monthlymaxvmax'] = vmax
        obstc.loc[idx, 'monthlymaxpmin'] = pmin
